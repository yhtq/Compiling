...
import Utils (Located (Located), unlocated, Into (into))
...
toKeyword :: (S.Tokens s ~ T.Text) => S.Tokens s -> Maybe Keywords
toKeyword = __toKeyword

type LexcialR' = LexcialR T.Text Keywords

__refineLexcial :: forall s. (S.Tokens s ~ T.Text) => Located (Lexcial s) -> Maybe (Located LexcialR')
__refineLexcial (Located (x, pos)) = do
    kw <- refineLexcial (toKeyword @s) x
    case kw of
        NumericLiteralR n -> Just $ Located (NumericLiteralR n, pos)
        StringLiteralR s -> Just $ Located (StringLiteralR s, pos)
        KeywordR k -> Just $ Located (KeywordR k, pos)
        IdentifierR k -> Just $ Located (IdentifierR k, pos)
...
type Buf = (S.Tokens LexerStream, S.Tokens LexerStream) -- (已读, 待读)

runLexerStream :: (S.Tokens s ~ T.Text, State Buf :> es, Input (Located (Lexcial s)) :> es) => ParseEff s err (S.Stream LexerStream Buf : es) a -> ParseEff s err es a
...
type ParseCons buf s st err es = (
        ParseEffFOEWithTokens buf s st err es, TShow (S.Token s), Into (S.Token s) LexcialR'
    )

parseKeyword :: forall s buf st err es. (ParseCons buf s st err es) => Keywords -> ParseEff s err es ()
...
parseParened :: (ParseCons buf s st err es) =>
...
parseLitTy :: (ParseCons buf s st err es) => ParseEff s err es (Typ TypeLitO TyVar)
...
parseTyVar :: (ParseCons buf s st err es) => ParseEff s err es (Typ TypeLitO TyVar)
...
parseTy :: (ParseCons buf s st err es) => ParseEff s err es (Typ TypeLitO TyVar)
...
parseTermVar :: (ParseCons buf s st err es, IsText t) => ParseEff s err es t
parseTermVar = satisfy'_  (\case
        IdentifierR s ->  Just (fromText s)
        _ -> Nothing
        )

parseLit :: (ParseCons buf s st err es, LCTerm t) => ParseEff s err es t
parseLit = satisfy'_  (\case
...
parseAnno :: (ParseCons buf s st err es) => ParseEff s err es (Anno TyVar TrmVar)
parseAnno = withInStack' "When parsing annotation" do
    var <- parseTermVar
    parseKeyword TypeAnnot
    Anno (TrmVar var) <$> parseTy

parseLam :: (ParseCons buf s st err es) => ParseEff s err es PTerm
...
parseBind :: forall buf s st err es. (ParseCons buf s st err es) => ParseEff s err es (Bind TyVar TrmVar)
parseBind = withInStack' "When parsing bind" do
    rec_ <- optional (parseKeyword @_ @s Rec)
    var <- parseTermVar
    parseKeyword Equal
    body <- parseExp
    case rec_ of
...
parseExp :: (ParseCons buf s st err es) => ParseEff s err es PTerm
parseExp = withInStack' "When parsing expression" do
    ...
    where
        parseLet :: (ParseCons buf s st err es) => ParseEff s err es PTerm
        parseLet = do
            parseKeyword Let
            bind <- parseBind
            parseKeyword In
...
newtype ParserError = ParserError T.Text deriving (Show, Eq, IsText, TShow)

type ParserES = '[
        S.Stream LexerStream Buf,
        Input (Located (Lexcial T.Text)),
        MultiThrow (ParserError, Buf),
        State Buf
        ]
